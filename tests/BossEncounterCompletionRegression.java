...
public final class BossEncounterCompletionRegression {
 ...
 static void groundTiming()throws Exception{
  clean();CorporealBeast n=new CorporealBeast(8133);n.setLocation(Location.locate(2980,4380,2));n.setOriginalLocation(n.getLocation());World.getWorld().getNpcs().add(n);landscape(2980,4380);Player p=player(n);p.setLocation(Location.locate(2991,4380,2));p.updateRegionArea();packets.get(p).clear();n.getRandom().setSeed(812);
  EncounterAttack attack=new EncounterAttack(n,EncounterAttack.Kind.CORP_SPLIT);attack.setInteraction(new Interaction(n,p));check(attack.commenceSession(),"Corp ground attack launches");
  org.dementhium.net.message.Message projectile=packets.get(p).stream().filter(m->m.getOpcode()==15&&m.getBuffer().getUnsignedShort(9)==EncounterAttack.Kind.CORP_SPLIT.projectile).findFirst().orElse(null);check(projectile!=null,"Corp ground projectile reaches nearby player");org.jboss.netty.buffer.ChannelBuffer data=projectile.getBuffer();check(data.getUnsignedShort(13)==0&&data.getUnsignedShort(15)==90,"Ground projectile encodes zero delay and three-tick duration");
  int hp=p.getHitPoints();cycle();cycle();check(p.getHitPoints()==hp,"Ground damage waits for projectile arrival");cycle();check(p.getHitPoints()<hp,"Ground damage lands with projectile at three ticks");
 }
 ...
 public static void main(String[] args)throws Exception{CombatFixtures.init();List<String> failures=new ArrayList<String>();String[] groups=args.length==0?new String[]{"assets","doors","core","groundTiming","nex"}:args;for(String group:groups)try{if(group.equals("assets"))assets();else if(group.equals("doors"))doors();else if(group.equals("core"))core();else if(group.equals("groundTiming"))groundTiming();else if(group.equals("nex"))nex();else throw new IllegalArgumentException("Unknown group: "+group);}catch(Throwable failure){failures.add(group+": "+failure);failure.printStackTrace();}if(!failures.isEmpty())throw new AssertionError(failures.toString());System.out.println("Boss encounter completion: "+checks+" checks passed");}
}
