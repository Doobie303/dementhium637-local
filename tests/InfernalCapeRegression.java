...
public class InfernalCapeRegression extends DuelRegression {
 ...
 public static void main(String[] args)throws Exception {
  ...
  ItemDefinition d=ItemDefinition.forId(InfernalCape.ID);
  ...
  Commands.handle(admin,new String[]{"infernalcape"});check(count(admin.getInventory().getContainer(),InfernalCape.ID)==0,"old client cannot spawn custom item");
  ...
  Player ordinary=p();ordinary.getConnection().readClientSettings("|infernal-cape=1|gambler-ui=1");Commands.handle(ordinary,new String[]{"infernalcape"});check(count(ordinary.getInventory().getContainer(),InfernalCape.ID)==0,"admin-only grant");
  Commands.handle(admin,new String[]{"infernalcape"});check(count(admin.getInventory().getContainer(),InfernalCape.ID)==1,"admin grant");
  admin.getEquipment().equip(admin,0,0,InfernalCape.ID,true);check(admin.getEquipment().get(Equipment.SLOT_CAPE).getId()==InfernalCape.ID,"equips in cape slot");check(count(admin.getInventory().getContainer(),InfernalCape.ID)==0,"equip consumes inventory copy");
  for(int i=0;i<15;i++)check(admin.getBonuses().getBonus(i)==d.getBonus()[i],"equipped bonus "+i); Player old=p();
  for(byte gender=0;gender<2;gender++){admin.getAppearance().setGender(gender);check(appearance(admin,admin)==InfernalCape.EQUIP_ID,"capable viewer actual appearance packet");check(appearance(old,admin)==ItemDefinition.forId(6570).getEquipId(),"old viewer actual appearance packet fallback");}
  check(World.getWorld().getPlayerLoader().save(admin),"save custom equipment");Player loaded=reload(admin);check(loaded.getEquipment().get(Equipment.SLOT_CAPE).getId()==InfernalCape.ID,"custom equipment persisted");
  old.getInventory().getContainer().set(0,new Item(InfernalCape.ID));old.getEquipment().equip(old,0,0,InfernalCape.ID,true);check(old.getEquipment().get(Equipment.SLOT_CAPE)==null&&count(old.getInventory().getContainer(),InfernalCape.ID)==1,"old client equip rejects without loss");
