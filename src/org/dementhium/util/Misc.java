...
public class Misc {
	...
	public static String formatYellText(String yellText) {
		if (yellText == null)
			return "";
		yellText = yellText.replaceAll("_", " ");
		yellText = yellText.toLowerCase();
		StringBuilder newYellText = new StringBuilder();
		boolean wasDot = true;
		for (int i = 0; i < yellText.length(); i++) {
			if (wasDot) {
				newYellText.append(("" + yellText.charAt(i)).toUpperCase());
				wasDot = false;
			} else {
				newYellText.append(yellText.charAt(i));
			}
			if (yellText.charAt(i) == '.') {
				wasDot = true;
			}
		}
		return newYellText.toString();
	}
