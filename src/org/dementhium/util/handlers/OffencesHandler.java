...
public class OffencesHandler {
    ...
    public boolean isIpBanned(Player p) {
        ...
        if (bannedIps.contains(formatIp(p.getConnection().getChannel().getRemoteAddress().toString()))) {
            ipBanned = true;
        }
        return ipBanned;
    }
    ...
    public void unMute(String user, boolean ipMute) {
        if (ipMute) {
            mutedIps.remove(user); //user means ip address now!
        } else {
    ...
    public void unBan(String p, boolean ipBan) {
        if (ipBan) {
            bannedIps.remove(p); //p means ip address now!
        } else {
    ...
    public ArrayList<String> getMutedIps() {
    	return mutedIps;
    }
