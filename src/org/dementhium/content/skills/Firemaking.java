...
public class Firemaking {
	...
	private static enum LightableLog {
		...
		ACHEY(2862, 1, 2, 70),
		OAK(1521, 15, 3, 150),
		WILLOW(1519, 6, 20, 200),
		TEAK(6333, 10, 36, 200),
		ARTIC_PINE(10810, 12, 50, 300),
		MAPLE(1517, 15, 70, 250),
		MAHOGANY(6332, 17, 85.5, 400),
		EUCALYPTUS(12581, 19, 94.5, 450),
		YEW(1515, 29, 120.5, 350),
		MAGIC(1513, 41, 190.8, 450);
	...
	public static boolean firemake(final Player player, int itemUsed, int usedWith, int usedSlot, int usedWithSlot, final boolean onGround) {
		...
		if (log != null) {
			...
			if ((!onGround && slot > -1 && player.getInventory().getContainer().contains(player.getInventory().get(slot))) || onGround) {
				...
				final GroundItem groundItem = onGround ? GroundItemManager.getQualifiedGroundItem(usedWith, player.getLocation(), player) : new GroundItem(player, item, player.getLocation(), false, false, GroundItemManager.groundItemIndex++);
				if (!onGround) {
								...
								if (GroundItemManager.getGroundItems().contains(groundItem)) {
									...
									World.getWorld().submit(new Tick(log.ticks) {
										...
										public void execute() {
											...
											GroundItemManager.createGroundItem(new GroundItem(player, new Item(592), fireLocation.getX(), fireLocation.getY(), fireLocation.getZ(), true, false, GroundItemManager.groundItemIndex++));
											this.stop();
										}
