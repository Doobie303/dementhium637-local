...
public class StallThieving extends SkillAction {
	...
	public boolean commence(Player player) {
        if (player.getSkills().getLevelForExperience(Skills.THIEVING) < stall.getThievingLevel()) {
	...
	public boolean finish(Player player) {
		...
		if (!succesful) {
			...
			for (NPC npc : guards) {
				...
				npc.setAttribute("guardAttacking", true);
		...
		player.sendMessage("You steal "+(item.getDefinition().getName().equals("Coins") ? "some" : "a")+" "+item.getDefinition().getName()+".");
		if (player.getSkills().getLevelForExperience(Skills.THIEVING) == 99 && item.getId() == 995)
			player.getInventory().addItem(item.getId(), (int) Math.round(item.getAmount()*1.5));
		else
