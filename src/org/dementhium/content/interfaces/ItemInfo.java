...
public class ItemInfo {
    ...
    public static void sendItemInfo(Player p, Item item, int slot, int currency) {
        if (item != null) {
            ...
            if (item.getId() == 10564)
            	...
            	ActionSender.sendBConfig(p, 743, currency == -1 ? 1464 : currency);
            ...
            int amount = p.getPkPoints();
            if (item.getId() == 10564)
            	Misc.amountToString(p.getInventory().getContainer().getItemCount(995));
            else if (currency != -1)
            	Misc.amountToString(p.getInventory().getContainer().getItemCount(currency));
            String currencyName;
            if (item.getId() != 10564)
            	currencyName = "coins";
            else if (currency == -1)
        		...
        		currencyName = ""+ItemDefinition.forId(currency).getName();
            ActionSender.sendString(p, 449, 25, "<col=ff9900>You have "+amount+" "+currencyName+".");
