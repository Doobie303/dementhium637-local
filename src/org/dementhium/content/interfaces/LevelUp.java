...
public class LevelUp {
    ...
    public static final Graphic SKILL_MASTERY_FIREWORKS[] = {Graphic.create(1633), Graphic.create(1634), Graphic.create(1635), Graphic.create(1637)};
    ...
    public static void levelUp(Player player) {
        for (int i = 0; i < player.getSettings().getLeveledUp().length; i++) {
            if (player.getSettings().getLeveledUp()[i]) {
            	...
            	else if (player.getSkills().getLevelForExperience(i) >= 99) {
            		int gfx = Misc.random(3);
            		player.graphics(SKILL_MASTERY_FIREWORKS[gfx]);
            		if (SkillCapes.getNumberOf99s(player) > 1)
            			SkillCapes.trimSkillCape(player);
    				if (player.getRights() < 2) {
    					for (Player pl : World.getWorld().getPlayers()) {
    						if (pl != player) {
    							pl.sendMessage("<col=FFA500><shad=000000>News: "+player.getDisplayName()+" has achieved "+player.getSkills().getLevelForExperience(i)+" "+Skills.SKILL_NAME[i]+".");
    						}
    					}
    				}
            	}	
        ...
        int spriteConfig = 0;
        for (int i = 0; i < player.getSettings().getLeveledUpConfig().length; i++) {
            if (player.getSettings().getLeveledUpConfig()[i]) {
        ...
        for (int i = 0; i < player.getSettings().getLeveledUp().length; i++) {
            if (player.getSettings().getLeveledUp()[i]) {
        		String name = Skills.SKILL_NAME[i];
                ActionSender.sendMessage(player, "You've just advanced a"+ (name.startsWith("A") ? "n": "")+" " + Skills.SKILL_NAME[i] + " level! You have reached level " + player.getSkills().getLevelForExperience(i) + ".");
                ActionSender.sendString(player, "Congratulations, you have just advanced a"+ (name.startsWith("A") ? "n": "")+" " + Skills.SKILL_NAME[i] + " level!", 740, 0);
