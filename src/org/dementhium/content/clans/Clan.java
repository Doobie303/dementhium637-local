...
public class Clan {
    ...
    public Clan(String owner, String ownerDisplayName, String clanName) {
        this.roomName = clanName;
        this.roomOwner = owner;
        this.roomOwnerDisplayName = ownerDisplayName;
        setTransient();
    }
    ...
    public String getOwnerDisplayName() {
        return roomOwnerDisplayName;
    }

    public void setOwnerDisplayName(String displayName) {
        this.roomOwnerDisplayName = displayName;
    }
    ...
    public Byte getRank(Player player) {
        if (Misc.formatPlayerNameForProtocol(player.getUsername()).equals(roomOwner)) {
            ...
            return 127;
        } else if (ranks.containsKey(player.getDisplayName())) {
            return ranks.get(player.getDisplayName());
    ...
    public boolean canJoin(Player player) {
        ...
        if (banned.containsKey(player.getDisplayName()) || banned.containsKey(player.getUsername())) {
            if (System.currentTimeMillis() - banned.get(player.getDisplayName()) < 3600000 || System.currentTimeMillis() - banned.get(player.getUsername()) < 3600000) {
                player.sendMessage("You have been banned from this channel.");
                return false;
            } else {
                banned.remove(player.getDisplayName());
            }
        }
        if (ranks.containsKey(player.getDisplayName())) {
            rank = (byte) (ranks.get(player.getDisplayName()) + 1);
            return rank >= joinReq;
        } else if (ranks.containsKey(player.getUsername())) {
