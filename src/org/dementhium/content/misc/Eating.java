    ...
    public boolean interfaceOption(final Player player, int interfaceId, int buttonId, int slot, int itemId, int opcode) {
        ...
        if (food != null) {
            ...
            if (player.getAttribute("consumed") != Boolean.TRUE) {
                ...
                if (player.getSkills().getHitPoints() < player.getSkills().getMaximumLifePoints())
                	ActionSender.sendChatMessage(player, 0, "It restores some life points.");
                ...
                if (food.getId() == 15272) { //Rocktails heal 100 lp more in real RS.
