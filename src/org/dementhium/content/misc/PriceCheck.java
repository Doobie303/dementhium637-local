...
public class PriceCheck {
	...
	public boolean close() {
		...
		player.getInventory().addAllDropable(container);
		container.clear();
	...
	private boolean refresh(boolean reArrange) {
		...
		for (Item item : container.toArray()) {
			if (item != null) {
				value = item.getDefinition().getExchangePrice() == 0 ? 1 : item.getDefinition().getExchangePrice();//.getStorePrice();
				totalValue += value * item.getAmount();
				double itemAmount = item.getAmount();
				double maxAmount = (Integer.MAX_VALUE / value);
				if (itemAmount > maxAmount || totalValue < 0) { //both checks are needed! (first one mainly for stackables and the other for multiple non-stackables)
					ignoreValue = true;
				}
			}
