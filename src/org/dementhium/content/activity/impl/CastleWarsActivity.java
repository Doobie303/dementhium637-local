...
import org.dementhium.content.activity.impl.castlewars.CastleWarsItemActions;
import org.dementhium.content.activity.impl.castlewars.CastleWarsObjects;
import org.dementhium.content.activity.impl.castlewars.DynamicObject;
import org.dementhium.content.activity.impl.castlewars.DynamicObject.ActionType;
import org.dementhium.content.activity.impl.castlewars.TeamManager;
import org.dementhium.content.activity.impl.castlewars.TeamManager.FlagState;
import org.dementhium.content.activity.impl.castlewars.TeamType;
...
public class CastleWarsActivity extends Activity<Player> {
    ...
    public Player currentFlagHolderOfTeamSaradomin;
    public Player currentFlagHolderOfTeamZamorak;
    ...
    private void updatePlayers() {
        ...
        if (gameTicks < 1) { //means that the castle wars game ended
        	boolean tie = false;
        	boolean saraTeamWins = false;
        	if (saradominTeam.getPoints() == zamorakTeam.getPoints())
        		tie = true;
        	else if (saradominTeam.getPoints() > zamorakTeam.getPoints())
        		saraTeamWins = true;
            ...
            for (Player player : players) {
                ...
                player.removeTick("death_tick");
                removeItems(player);
                player.fullRestore();
                player.teleport(2440 + Misc.random(4), 3083 + Misc.random(12), 0, false);
                ActionSender.sendCloseOverlay(player);
                player.setActivity(Mob.DEFAULT_ACTIVITY);
                if (tie) {
                	player.getInventory().addDropable(new Item(4067, 100));
                	player.getInventory().refresh();
                } else if (saraTeamWins && saradominTeam.getPlayers().contains(player)) {
                	player.getInventory().addDropable(new Item(4067, 200));
                	player.getInventory().refresh();
                } else if (!saraTeamWins && zamorakTeam.getPlayers().contains(player)) {
                	player.getInventory().addDropable(new Item(4067, 200));
                	player.getInventory().refresh();
                }
            }
    ...
    private void resetGameField() {
        ...
        currentFlagHolderOfTeamZamorak = null;
        ...
        currentFlagHolderOfTeamSaradomin = null;
    ...
    private void updateWaitingRooms() {
        if (minutesLeft < 1) {
            ...
            for (Player player : zamorakTeam.getWaitingPlayers()) {
                ...
                player.teleport(2369 + Misc.random(6), 3128 + Misc.random(6), 1, false);
                ActionSender.sendPlayerOption(player, "Attack", 1, true);
                ActionSender.sendPlayerOption(player, "Heal", 2, false);
                ActionSender.sendPlayerOption(player, "Take-from", 5, false);
                player.getInventory().addItem(590, 1);
            }
            for (Player player : saradominTeam.getWaitingPlayers()) {
                ActionSender.sendOverlay(player, 58);
                player.teleport(2424 + Misc.random(6), 3073 + Misc.random(6), 1, false);
                ActionSender.sendPlayerOption(player, "Attack", 1, true);
                ActionSender.sendPlayerOption(player, "Heal", 2, false);
                ActionSender.sendPlayerOption(player, "Take-from", 5, false);
                player.getInventory().addItem(590, 1);
    ...
    public synchronized void register(Player player, int objectId) {
        ...
        if (player.getFamiliar() != null && player.getFamiliar().isBeastOfBurden()) {
            player.sendMessage("Please dismiss your beast of burden before entering the minigame.");
            return;
        }
        if (player.getEquipment().getSlot(Equipment.SLOT_WEAPON) == 4037 || player.getEquipment().getSlot(Equipment.SLOT_WEAPON) == 4039
        ...
        if (objectId == 4388) {
            ...
            player.teleport(2420 + Misc.random(4), 9517 + Misc.random(8), 0, false);
            ...
            player.teleport(2376 + Misc.random(6), 9485 + Misc.random(4), 0, false);
    ...
    public void removeItems(Player player) {
        ActionSender.sendPlayerOption(player, "null", 1, true); //Attack
    	ActionSender.sendPlayerOption(player, "null", 2, false); //Heal
        ActionSender.sendPlayerOption(player, "null", 5, false); //Take-from
        player.removeAttribute("flagTakenStartTime");
        player.getEquipment().set(Equipment.SLOT_CAPE, null);
        player.getInventory().deleteItem(590, 1);
        ...
        if (action.equals("ItemOption") && actionId == 1) {
            if (item.getId() == 4053) {
                if (World.getWorld().getAreaManager().getAreaByName("NorthRocks").contains(player.getLocation())
                        ...
                        || ((World.getWorld().getAreaManager().getAreaByName("SaradominRespawnRoom").contains(player.getLocation())
                        || World.getWorld().getAreaManager().getAreaByName("ZamorakRespawnRoom").contains(player.getLocation())) && player.getLocation().getZ() == 1)) {
                ...
                player.getSkills().heal(player.getSkills().getMaximumLifePoints() / 10);
    ...
    public boolean isCombatActivity(Mob mob, Mob victim, boolean sendMessages) {
        ...
        if (zamorakTeam.getPlayers().contains(mob.getPlayer()) && zamorakTeam.getPlayers().contains(victim.getPlayer())) {
			if (sendMessages)
				mob.getPlayer().sendMessage("That player is on your side. Don't attack them!");
            return false;
        }
    ...
    public boolean onTeleport(Player player) {
        player.sendMessage("You can't teleport out of Castle Wars.");
        return false;
    }
    ...
    public boolean canLogout(Player player, boolean logoutButton) {
        ...
        if (zamorakTeam.getDisconnectedPlayers().contains(player.getUsername() )
        		|| saradominTeam.getDisconnectedPlayers().contains(player.getUsername())) //means that we are checking this method for the 2nd time.
        	return true;
        removeItems(player);
        player.teleport(2440 + Misc.random(4), 3083 + Misc.random(12), 0, false);
