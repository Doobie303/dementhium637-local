...
public class PlayerRestorationTick extends Tick {
    ...
    public void execute() {
        if (player.isOnline()) {
            if (player.getSpecialAmount() < 1000) {
                ...
                if (specialRestoration == 0) {
                    ...
                    if (player.getSpecialAmount() + 100 >= 1000) {
                        ...
                        player.sendMessage("<col=64FE2E><shad=000000>Your special attack energy is now 100%.");
                    } else {
                        player.setSpecialAmount(player.getSpecialAmount() + 100);
                        if (player.getSpecialAmount() - 100 < 500 && player.getSpecialAmount() >= 500)
                        	player.sendMessage("<col=64FE2E><shad=000000>Your special attack energy is now 50%.");
                    }
                }
            }
            ...
            if (player.getAttribute("overloads") == Boolean.TRUE) {
                ...
                if (overloadTick == -1) {
                    for (int i = 0; i < Skills.SKILL_COUNT; i++) {
                        if (i == Skills.PRAYER || i == Skills.SUMMONING || i == Skills.CONSTITUTION) {
                            continue;
                        }
                    ...
                    player.sendMessage("The effects of overload have worn off, and you feel normal again.");
                }
            ...
            if (healTick == 0) {
                healTick = 10;
                if (player.getSkills().getHitPoints() < player.getSkills().getLevelForExperience(Skills.CONSTITUTION) * 10 || player.getSkills().getHitPoints() < player.getSkills().getMaximumLifePoints()) {
                    player.getSkills().heal(1);
                } else if (player.getSkills().getHitPoints() > player.getSkills().getMaximumLifePoints())
                	player.getSkills().decreaseLifePointsTick(1);
            }
        } else {
            this.stop();
        }
