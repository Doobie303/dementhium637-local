...
/*
 * I made this file unharmfull... prolly a good file if you need it but the damage is big when it starts to run when you don't want it to =(
 */
	...
	/*private static void saveDefinitions() {
		...
		try {
			...
			for (int x = 0; x < ItemDefinition.MAX_SIZE; x++) {
				...
				raf.writeBytes(itemDef.isNoted() ? "Swap this note at any bank for the equivalent item." : itemDef.getExamine(1) != null ? itemDef.getExamine(1) : "It's a " + itemDef.getName()+".");
				raf.writeByte(0);
				if (!itemDef.isNoted()) {
