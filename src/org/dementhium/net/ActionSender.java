...
public class ActionSender { // 2370 -( 2380, 9360+, 9570+
	...
	public static void moveCamera(Player p, int speed,
		...
		bldr.writeByteS(floorHeight); 
		bldr.writeByteA(sideToSideDistance);
		bldr.writeShort(cameraHeight); 
	...
	public static void sendPublicChatMessage(Player player, int playerIndex,
		...
		if (chat instanceof QuickChatMessage) {
			bldr.writeByte(chat.getChatText() == null ? 2 : chat.getChatText().getBytes().length + 2);
			bldr.writeShort(((QuickChatMessage) chat).getFileId());
			if(chat.getChatText() != null)
				bldr.writeBytes(chat.getChatText().getBytes());
		} else {
	...
	public static void sendFriend(Player player, String Username,
		...
		if (writeOnline) {
			bldr.writeRS2String(isLobby ? "<col=00FF00>Lobby"
					: "<col=00FF00>"+Constants.SERVER_NAME + "");
			bldr.writeByte(0);
		}
		player.write(bldr.toMessage());
	}
	...
	public static void sendIgnore(Player player, String name, String displayName) {
		...
		if (displayName.equals(name))
			name = "";
		bldr.writeRS2String(""+displayName);
		bldr.writeRS2String(""+displayName); //change this to name?
    ...
    public static void sendPrivateChatSetting(Player player, int setting) {
    	MessageBuilder bldr = new MessageBuilder(66);
        bldr.writeByte(setting);
        player.write(bldr.toMessage());
    }
	...
	public static void receivePrivateMessage(Player player, String username,
		...
		bldr.writeByte(username.equals(displayName) ? 0 : 1); // has a previous name?
		bldr.writeRS2String(""+displayName);
		if (!username.equals(displayName))
			bldr.writeRS2String(""+username);
	...
	public static void sendOtherLoginPackets(Player player) {
		...
		//-walk here-
		//2 = heal (castle wars)
		ActionSender.sendPlayerOption(player, "Follow", 3, false);
		ActionSender.sendPlayerOption(player, "Trade with", 4, false);
		//5 = Take-from (castle wars)
		ActionSender.sendPlayerOption(player, "Req Assist", 6, false);
		if (player.getRights() > 1)
			ActionSender.sendPlayerOption(player, "Administrate", 7, false);
		sendMessage(player, "Welcome to DyNamics.");
		//sendMessage(player, "::master - ::pure - ::tank - ::barrowsgear - ::gear - ::supplies");
		//player.setDoubleXpTimer(System.currentTimeMillis() + 720000000);
		if (player.isDoubleXp()) {
			//sendMessage(player, "<col=ff0000>Last FULL reset, due too duping ect. 1-2 weeks of bonus XP!");
			sendMessage(player, "<col=ff0000>You are currently on Bonus XP!");
			sendMessage(player, "<col=ff0000>You have " + player.getTimeLeft()
					+ " minuets left of your bonus experience!");
		}
		...
		sendConfig(player, 300, player.getSettings().getSpecialAmount());
			...
			p.sendMessage("<col=FF0000><shad=000000>Welcome " + Misc.formatPlayerNameForDisplay(player.getUsername())+ " a new player of DyNamic's!");
			}
		}
	}
	...
	/*
	 * Trade Exclamation Marks
	 * 
	 * @param player The Player
	 * @param positionRight {@code True} The exclamation mark (!) will be shown on the right side., {@code false} The exclamation mark (!) will be shown on the left side.. 
	 */
	public static void sendTradeModified(Player player, boolean positionRight,int TradeSlot) {
		if (positionRight) {
			Object[] tparams4 = new Object[]{TradeSlot, 7, 4, 21954593};
			sendClientScript(player, 143, tparams4, "Iiii");
		} else { //showExclamationMarkOnLeftSide
			Object[] tparams4 = new Object[]{TradeSlot, 7, 4, 21954591};
			sendClientScript(player, 143, tparams4, "Iiii");
		}
	}
	
	//Un-Tested method:
	public static void sendItemLendingModified(Player player, boolean positionRight,int TradeSlot) {
		if (positionRight) {
			Object[] tparams4 = new Object[]{TradeSlot, 21954608};
			sendClientScript(player, 714, tparams4, "Iiii");
		} else { //showExclamationMarkOnLeftSide
			Object[] tparams4 = new Object[]{TradeSlot, 21954607};
			sendClientScript(player, 714, tparams4, "Iiii");
		}
		Object[] tparams4 = new Object[]{TradeSlot, -1};
		sendClientScript(player, 146, tparams4, "Iiii");
	}
	...
	public static void sendCloseOverlay(Player player) {
		...
		int slotId = 7; //This fixes closeOverlay in resizeable screen mode.
		sendCloseInterface(player, winId, slotId);
	}
	...
	public static void loginResponse(Player player) {
		...
		// player.getDefinition().setRights(0);
		//
		for(String admin : PlayerLoader.administrators) {
			if(player.getUsername().equalsIgnoreCase(admin)) {
				player.getDefinition().setRights(2);
			}
		}
		for(String pmod : PlayerLoader.playerModerators) {
			if(player.getUsername().equalsIgnoreCase(pmod)) {
				player.getDefinition().setRights(1);
			}
		}
		for(String smod : PlayerLoader.superMods) {
			if(player.getUsername().equalsIgnoreCase(smod)) {
				player.getDefinition().setRights(2);
			}
		}
	...
	public static void sendLogout(Player player, int button) {
		if (player.getCombatExecutor().getLastAttacker() != null) {
			player.sendMessage("You can't logout until 10 seconds after the end of combat.");
			return;
		}
		if (!player.getActivity().canLogout(player, true)) {
			return;
		}
		//World.getWorld().getPlayerLoader().save(player); //done in unregister (which is called in SessionLogoutTask.java)
		MessageBuilder bldr = new MessageBuilder(button == 5 ? 45 : 23);
		player.write(bldr.toMessage());
		//World.getWorld().unregister(player); //SessionLogoutTask.java does this, doing it double bugs familiars!
	}
	...
	public static void CreatePouchOptions(Player player) {
		ActionSender.sendInterface(player, 672);
		Object[] tparams1 = new Object[]{78, 1, "List<col=FF9040>", "Infuse-X<col=FF9040>", "Infuse-All<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-5<col=FF9040>", "Infuse<col=FF9040>", 10, 8, 672 << 16 | 16};
		sendClientScript(player, 757, tparams1, "Iiissssssii");
		sendAMask(player, 190, 672, 16, 0, 462);
		sendConfig(player, 448, 3);
	}
	
	public static void CreateScrollOptions(Player player) {
		ActionSender.sendInterface(player, 666);
		Object[] tparams1 = new Object[]{78, 1, "Transform-X<col=ff9040>", "Transform-All<col=ff9040>", "Transform-10<col=ff9040>", "Transform-5<col=ff9040>", "Transform<col=ff9040>", 10, 8, 666 << 16 | 16};
		ActionSender.sendClientScript(player, 763, tparams1, "Iiisssssii");
		ActionSender.sendAMask(player, 126, 666, 16, 0, 462);
	}
	...
	public static void sendLobbyResponse(Player player) {
		...
		short today = (short) (System.currentTimeMillis() / 1000 / 60 / 60 / 24);
		if (player.getRawLastConnectDate() == -1 || player.getLastConnectIp().equals("null")) {
			bldr.writeShort(0); //last login date
			int ipHash = Misc.IPAddressToNumber("127.0.0.1");
			bldr.writeInt(ipHash); //last ip
		} else {
			short lastLogin = (short) ((System.currentTimeMillis() - player.getRawLastConnectDate()) / 1000 / 60 / 60 / 24);
			if (lastLogin < 0) { //this means that the player has been messing with the dates on his pc ;), we do still want to give a valid responce to this player, not just a bugged date.
				bldr.writeShort(0); //last login date
			} else {
				DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
				String lastConnectDate = dateFormat.format(player.getRawLastConnectDate());
				String todaysDate = dateFormat.format(System.currentTimeMillis());
				if (lastLogin < 1 && !lastConnectDate.equals(todaysDate)) //if lastLogin is within 24 hours but is not the same day
					bldr.writeShort(today-11746-lastLogin);
				//The higher the 11745 is, the longer ago you would've logged in.
				//The 11745 is a value that goes up everyday (I don't know what causes this, probably somewhere in the client). 
				//Today is also a value that goes up everyday. 
				//By doing 'today - 11745', you are eliminating the thing in the client that makes the value go up.
				else
					bldr.writeShort(today-11745-lastLogin); //3229 - lastDays //(my value: 3512)
			}
			int ipHash = Misc.IPAddressToNumber(player.getLastConnectIp());
			bldr.writeInt(ipHash); //last ip
		}
