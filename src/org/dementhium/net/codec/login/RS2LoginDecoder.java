        ...
        switch (state) {
            ...
            case LOBBY_FINALIZATION:
                if (buffer.readable()) {
                    ...
                    long cacheVersion = buffer.readLong(); // client key
                    ...
                    boolean alreadyInLobby = false;
                    
                    for(Player player : World.getWorld().lobbyPlayers){
                    ...
                    if(alreadyInLobby){
                    	...
                    	session.write(new MessageBuilder().writeByte(5).toMessage()).addListener(ChannelFutureListener.CLOSE);
                        return null;
                    } else {
                    ...
                    if (cacheVersion == 712)
                    	World.getWorld().load(session, new PlayerDefinition(name, password), cacheVersion);
                    else
