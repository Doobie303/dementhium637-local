...
public class FairyRing extends EventListener {
	...
	public void register(EventManager manager) {
		...
		manager.registerObjectListener(27254, this);
	...
	public boolean interfaceOption(Player player, int interfaceId, int buttonId, int slot, int itemId, int opcode) {
		if (interfaceId == 734) {
			if (buttonId == 19) { //close
				...
				switch (hash) {
				case 1: // DIQ - Slayer Tower
					fairyRingTeleport(player, Location.locate(3428, 3536, 0), true);
					return true;
				case 195: // BIQ - Kharidian Desert: Near Kalphite hive
					
					return true;
				case 192: // AIQ - Asgarnia: Mudskipper Point
					fairyRingTeleport(player, Location.locate(2996, 3115, 0), true);
					return true;
				case 2: // CIP - Taverly Dungeon
					fairyRingTeleport(player, Location.locate(2884, 9798, 0), true);
					return true;
				case 155: // BJR - P2P side of Karamja
					fairyRingTeleport(player, Location.locate(2743, 3239, 0), true);
					return true;
				case 145: // DKR - Misthalin: Edgeville
					fairyRingTeleport(player, Location.locate(3129, 3496, 0), true);
					return true;
				case 80: // AKS - Feldip Hills: Feldip Hunter area
					fairyRingTeleport(player, Location.locate(2571, 2956, 0), true);
					return true;
				case 66: // CIS - Dungeon area
					fairyRingTeleport(player, Location.locate(3168, 9572, 0), true);
					return true;
				case 72: // ALS - Kandarin: McGrubor's Wood
					fairyRingTeleport(player, Location.locate(2644, 3495, 0), true);
					return true;
				case 3: // BIP - Skeletal horror cave.
					fairyRingTeleport(player, Location.locate(3637, 9695, 0), true);
					return true;
				case 11: // BLP - Tzhaars
					fairyRingTeleport(player, Location.locate(2437, 5126, 0), true);
					return true;
				case 89://Zanaris DJS
					fairyRingTeleport(player, Location.locate(2412, 4435, 0), true);
					return true;
				case 152://AJR - Fremennik Slayer Dungeon
					fairyRingTeleport(player, Location.locate(2807, 10002, 0), true);
					return true;
				}
	...
	public static void fairyRingTeleport(final Player player, final Location to, boolean showWildiWarning) {
    	if (showWildiWarning) {
    		if (!player.isInWilderness() && (World.getWorld().getAreaManager().getAreaByName("Wilderness").contains(to)
    				|| World.getWorld().getAreaManager().getAreaByName("RevenantsCave").contains(to)
    				|| World.getWorld().getAreaManager().getAreaByName("FireGiantsDungeon").contains(to))) {
        		//if (player.get x,y,z == Wilderness && x,y,z != safeZone)
           		player.setAttribute("teleportingToWild", to);
           		player.setAttribute("teleportingToWildMethod", 7);
