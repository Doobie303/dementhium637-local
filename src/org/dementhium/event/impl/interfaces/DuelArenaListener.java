        ...
        if (interfaceId == 631) {
            ...
            switch (opcode) {
                case 6:
            		...
            		ActionSender.sendString(player, "" + other.getDisplayName(), 631, 23);
            		ActionSender.sendString(other, "" + player.getDisplayName(), 631, 23);
    ...
    private boolean handleChallengeOption(Player player, int buttonId) {
        switch (buttonId) {
            ...
            case 20:
                ...
                ActionSender.sendDuelReq(World.getWorld().getPlayers().get((Short) player.getAttribute("duelWithIndex")), player.getDisplayName(), "wishes to duel with you " + (player.getAttribute("isStaking") == Boolean.TRUE ? "(stake)" : "(friendly)") + ".");
