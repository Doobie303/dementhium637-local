...
public class CharacterDesignListener extends EventListener {
	...
	public static void closeChar(final Player player) {
		...
		if (player.hasSetAppearance() == false) {
			player.setHasSetAppearance(true);
			//player.setInvisible(false);
			player.turnTo(new NPC(8863), false);
			player.animate(4367);
	          World.getWorld().submit(new Tick(10) {
	                @Override
	                public void execute() {
	    				//Tutorial:
	    				DialogueManager.handle(player, new NPC(8863));
	                	stop();
	                }
	            });
