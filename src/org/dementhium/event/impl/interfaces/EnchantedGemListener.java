        ...
        if (itemId == 4155) {
            if (opcode == 13) {
                if (player.getSlayer().getSlayerTask() != null) {
                    player.sendMessage("Your current assignment is: " + player.getSlayer().getSlayerTask().getName().toLowerCase() + "s; only " + player.getSlayer().getSlayerTask().getCurrentTaskAmount() + " more to go.");
                } else {
