...
public class PlayerUpdate {
    ...
    public void loginData(MessageBuilder stream) {
        ...
        for (int index = 1; index < 2048; index++) {
            ...
            if (other == null || !other.isOnline() ||  other.isInvisible()) {
    ...
    public void sendUpdate() {
        ...
        if (localPlayers.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                if (other != null && other.isOnline() && !other.isInvisible() && other.getLocation().withinDistance(player.getLocation(), player.getViewDistance())) {
                    if (chatUpdate) {
        ...
        if (player.getViewDistance() > 0 && player.isActive()) {
            ...
            for (Player other : World.getWorld().getPlayers()) {
                ...
                if (other == player || added[other.getIndex()] || !player.getLocation().withinDistance(other.getLocation(), player.getViewDistance()) || !other.isOnline() || other.isInvisible()) {
                    continue;
                }
                addedCount++;
                packet.writeBits(1, 1);
    ...
    private void sendLocalPlayerTeleport(MessageBuilder packet, Player other) {
        ...
        Location last = other.lastLocation;
		Location delta = Location.getDelta(last, other.getLocation());
		...
		if (deltaX <= 15 && deltaY <= 15) {
			...
			deltaZ = delta.getZ();
			packet.writeBits(12, (deltaY & 0x1f) | ((deltaX & 0x1f) << 5) | ((deltaZ & 0x3) << 10));
		} else {
			packet.writeBits(1, 1); 
			packet.writeBits(30, (other.getLocation().getY() & 0x3fff) | ((other.getLocation().getX() & 0x3fff) << 14) | (other.getLocation().getZ() << 28));
		}
    }
    ...
    private void appendPlayerUpdateBlock(MessageBuilder updateBlock, Player other, boolean forceAppearance) {
        ...
        if (other.getMask().isAppearanceUpdate() || forceAppearance) {
            maskData |= 0x40;
        }
    ...
    public static void applyHitMask(Player player, Player other, MessageBuilder bldr) {
        ...
        for (DamageHit hit : other.getDamageManager().getHits()) {
            ...
            if (type != 9) {
                ...
                if (hit.getDamage() < 1 && !other.getSkills().isDead()) {
                    type = 8;
                }
    ...
    public void applyAppearanceMask(Player p, MessageBuilder updateMessage) {
		...
		if(p.hasDisplayName())
			hash |= 0x2;
        boolean inWild = p.isInWilderness() || p.inPVPZone();
        if (!inWild)
        	hash |= 0x4;
        appearanceData.writeByte(hash);
        int title = p.getTitle();
        appearanceData.writeByte(title); 
        if (p.getAttribute("superhit") != null)
        ...
        appearanceData.writeRS2String(p.getDisplayName());
