...
/*	public static List getRegionTiles(Location tile, int depth, int type) {
	...
	public int[][][] clippingMasks = new int[4][][];
	...
	public Location getLocalLocation(int x, int y, int z) {//wtf why are players getting -1 height? lol Idk man :P I'm dumb >.<
		z %= 4;
		if (tiles[z] == null) {
	...
	/**
	 * Only used for custom clipped tiles (I think). 
	 * Use 'getRegion().clippingMasks[z] != null' if you want to check if
	 * a normal region is clipped or not.
	 */
