...
public abstract class CombatAction {
	...
	public void execute() {
			...
			if (commenceSession()) {
				if (interaction.getSource().isPlayer()) {
					if (interaction.getSource().getPlayer().getUsername().equals("mod combat")) {
						interaction.getSource().getPlayer().sendMessage("[CombatAction.java] CASE: COMMENCE (3).");
					}
				}
		...
		case EXECUTE:
			if (interaction.getSource().isPlayer()) {
				if (interaction.getSource().getPlayer().getUsername().equals("mod combat")) {
					interaction.getSource().getPlayer().sendMessage("[CombatAction.java] CASE: EXECUTE (4).");
				}
			}
			if (executeSession()) {
				interaction.setState(CycleState.FINALIZE);
		...
		case FINALIZE:
			if (interaction.getSource().isPlayer()) {
				if (interaction.getSource().getPlayer().getUsername().equals("mod combat")) {
					interaction.getSource().getPlayer().sendMessage("[CombatAction.java] CASE: FINALIZE (5).");
				}
			}
			if (instant || getCombatType() == CombatType.MELEE) {
				executeSession();
