...
public class CombatMovement {
	...
	public static boolean combatFollow(Mob mob, Mob other, CombatType type) {
		...
		if ((type == CombatType.MELEE || type == CombatType.DEFAULT) && canMelee(mob, other)) {
			if (other.isNPC()) {
		        for (int npcId : NPC.NON_RETALIATING_NPCS) {
		        	if (other.getNPC().getId() == npcId) {
		        		return true;
		        	}
		        }
			}
			if (checkWall(mob, other)) {
			...
			if ((type == CombatType.MELEE || type == CombatType.DEFAULT) && mob.getWalkingQueue().isMoving()) {
				if (distance <= other.size()+2 && other.getWalkingQueue().isMoving())
					return true;
				else if (distance <= other.size()+3 && other.getWalkingQueue().isRunningMoving())
					return true;
			}
