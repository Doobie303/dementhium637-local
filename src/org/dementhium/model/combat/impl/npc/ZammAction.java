   ...
   public class ZammAction extends CombatAction {
      ...
      private static enum Style {
         PRIMARY(Graphic.create(1210, 96 << 16), 
         	...
         	Graphic.create(157, 96 << 16), 
            new CombatTask() {
               @Override
               public boolean execute(Interaction interaction) {
                  interaction.getSource().animate(6945);
                  return true;
               }
            }
         ),
         MAGIC(Graphic.create(1210, 96 << 16), 
         	Projectile.create(null, null, 1211, 30, 32, 52, 75, 3, 11), 
         	Graphic.create(157, 96 << 16), 
            new CombatTask() {
               @Override
               public boolean execute(Interaction interaction) {
                  final Mob zam = interaction.getSource();
                  ...
                  World.getWorld().submit(
                        new Tick(14) {
                           ...
                           public void execute() {
                              ...
                              victim.graphics(157);
                              zam.forceText("Let us fight to the death!");
                              if (victim.getPlayer().getPrayer().usingPrayer(1, 7)) {
                                 zam.getDamageManager().miscDamage(magicDamage, DamageType.DEFLECT);
                              }
