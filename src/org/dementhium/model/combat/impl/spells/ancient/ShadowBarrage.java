...
public class ShadowBarrage extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		...
		for (ExtraTarget m : interaction.getTargets()) {
			...
			if (m.getDamage().getHit() > -1 && m.getVictim().isPlayer() && m.getVictim().getRandom().nextInt(20) < 4) org.dementhium.model.combat.CombatStatus.shadowOnImpact(m.getDamage(),m.getVictim(),0.15);
	...
	public double getExperience(Interaction interaction) {
		...
		for (ExtraTarget e : interaction.getTargets()) {
			...
			if (e.getDamage().getHit() > 0) {
				interaction.getSource().getPlayer().getSkills().addExperience(Skills.CONSTITUTION, e.getDamage().getHit() * 0.133 * interaction.getSource().getPlayer().getPersonalCombatXpRate() / 100);
