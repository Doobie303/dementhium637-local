...
public class CorporealBeast extends EncounterNPC {
 private EncounterAdd core;private int nextCore,nextStomp;
 ...
 public boolean spawnCore(Player target){
  if(!NPCCombatContext.validPair(this,target)||!allows(target)||!target.isAttackable(this)||core!=null&&!core.isDead()&&!core.isHidden()||getOwningInstance()!=null)return false;
 ...
 @Override protected void mechanics(){
  ...
  if(clock()>=nextCore&&getHp()<getMaxHp()*3/4&&(core==null||core.isDead()||core.isHidden())){nextCore=clock()+10;if(!targets.isEmpty()&&getRandom().nextInt(4)==0)spawnCore(targets.get(getRandom().nextInt(targets.size())));}
  if(clock()<nextStomp)return;
  for(Player p:targets)if(EncounterNPC.gap(this,p)==0){nextStomp=clock()+6;animate(10057);int raw=getRandom().nextInt(301);Damage d=Damage.getDamage(this,p,CombatType.MELEE,raw,true);d.setMaximum(300);p.getDamageManager().damage(this,d,DamageType.MELEE);}
 }
}
