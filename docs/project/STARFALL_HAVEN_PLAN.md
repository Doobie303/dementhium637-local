...
The owner endorsed the spacious coastal concept, central classical statue, entrance arch and gold eagle, with **reuse of existing assets wherever possible and custom objects only where required**. The subsequent request explicitly authorizes custom statue and arch design/renders, matching banner artwork and necessary supporting custom design. This supersedes the earlier survey-before-design requirement for those named landmarks, while retaining reuse for supporting pieces and other scenery. It does not authorize home migration, map/cache deployment, staging, client publication or restart.

Current asset work: [editable landmark models v1](../../client/starfall-haven/models/v1/README.md), following the owner's explicit request to make the actual models/banners before map assembly and increase detail toward the design photo. Modular arch, gold eagle, classical statue, native-banner adaptations and SVG/PNG artwork are built, with OBJ/MTL, GLB, candidate DAT and actual-mesh renders. The existing client decoder matches all ten exports exactly; native rendering, performance and registered collision remain untested. These are development assets awaiting visual acceptance, not a Starfall release. The earlier [design v1 record](../../client/starfall-haven/design/v1/DESIGN_BRIEF.md) retains concept images, its historical generator failure and the post-reboot file checks.
...
Latest review: the owner approved the arch appearance and requested more statue face/chest detail and defined eagle feathers. That revision is saved with matching exports, close-up renders and validation, but the owner subsequently stated that the statue still does not resemble the original. **The statue is not artistically accepted.** Increasing procedural mesh detail has not solved its shape/likeness. Preserve the arch; change the statue-authoring approach before further cosmetic iteration.
...
This image is an AI concept, not a developer-client capture, surveyed placement sheet or collection of usable 3D models. Exact meshes, textures and proportions may change to suit existing cache assets. Preserve the composition, recognizable landmarks and sense of space; do not create new models merely to reproduce incidental image details.
...
- Central star: a classical Greek-style marble statue of an adult woman, topless with drapery around her hips and legs, on a low pedestal. Keep a walkable ring around its base.
...
No reuse percentage or exact custom-model count is established yet. The earlier estimate of three landmark projects is a possible upper-level work grouping, **not a commitment to build three new models**. A project can contain reused pieces and one small custom addition. Do not sacrifice the approved openness or silently remove the statue, eagle or arch to meet an arbitrary reuse quota; record a material design conflict for review.
...
| Central statue and pedestal | Custom figure and separate low plinth built within the authorized landmark scope; retain native-style face colours. | Continuous adult classical figure, sculpted hair and hip drapery. No additional supporting scenery family. | Editable mesh and three-view render saved; actual native decoding passes. Face/hands remain more stylized than the concept; visual/native-render/performance acceptance pending. |
...
- Keep passages and NPC approaches clear on all relevant sides. The arch cannot be represented by a single solid rectangle across its opening; align modular collision or prove an appropriate alternative. The statue base must block correctly while its surrounding route remains traversable.
...
4. **Build a minimal development preview.** Show actual bank/service footprints, arch plus eagle, statue, paths and outer bounds using selected assets. Tour with normal camera/roof settings and a few players before investing in detailed scenery or migrating home. Preserve the current home during this preview.
...
- Add focused map/model checks for affected archive loading, footprints, orientation, arch passage/side routes, statue collision, boundary crossings and relevant object visibility. Run live client checks for actual materials, animation where applicable, roofs, camera obstruction, clicks and cache delivery.
...
- Authorized now: actual custom statue/arch/eagle meshes, renders and banner artwork, with increased detail toward the approved photo; preserve references and reuse supporting scenery. Runtime migration remains outside this asset step.
- Changed contract: future asset selection must use the least custom work that preserves the required design and functionality. No executable behavior changed.
...
- Starfall asset state: editable meshes, actual renders, banner SVG/PNG and offline viewer built. Exact client-decoder/export checks and viewer smoke checks pass; see the asset README and validation JSON for coverage/durations. Statue face/hands and surface treatment remain more stylized than the reference. Gameplay implementation: not started. Automated gameplay verification: not run. Staged: no. Loaded: no Starfall release. Live accepted: no.
